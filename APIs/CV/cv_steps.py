...
class GetIconCoordinates(CV_Step):
	...
	def _get_icon_center(self, rect: tuple):
		return (
			rect[0][0] + (rect[1][0] - rect[0][0]) // 2,
			rect[0][1] + (rect[1][1] - rect[0][1]) // 2,
		)
	...
	def Do(self):
		...
		if max_val >= self.threshold:
			...
			self.logger.info(
				"Found icon {} at {}".format(
					self.icon_name, self.response.data.get("center", None)
				)
			)

			if self.debug_mode:
				cv2.rectangle(
					test, pt1=top_left, pt2=bottom_right, color=(255, 0, 0), thickness=4
				)
				cv2.imwrite(os.path.join(self.temp_dir, "result.png"), test)
				self.logger.debug(
					"Result image saved at {}".format(
						os.path.join(self.temp_dir, "result.png")
					)
				)
