...
class MakeMailAccounts(Mail_Step, BaseStep):
	def __init__(
		self,
		suggested_names: list = ["stephenhawking", "alberteinstein"],
		...
		self.suggested_names = suggested_names * (
			account_number // len(suggested_names)
		)
		self.__present_mails = self.__get_mails_from_bmd()

	def __get_mails_from_bmd(self):
		return [
			self._bmd.data.get(bot, None).get("EMAIL_KEY", None)
			for bot in self._bmd.data
		]
	...
	def Do(self):
		...
		for _mail in __mails:
			mail_creation_script(
				first=_mail["first"],
				last=_mail["last"],
				mail=_mail["mail"].split("@")[0],
			)
