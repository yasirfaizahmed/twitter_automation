...
def GetBotMetadata(botname: str = 'bot1', random: bool = False):
  warnings.warn("This method is legacy, and is Depricated, new version is in data_handler.data_handler.BotMetadata",
                DeprecationWarning,
                stacklevel=2)
  __bmd = BotMetadata()
  if random is False:
    ...
    if botname in __bmd.data.keys():
      return __bmd.data.get(botname, __bmd.data.get('default_bot', None))  # if botname is not found, return bot1 metadata
    else:
